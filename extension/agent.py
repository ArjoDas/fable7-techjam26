...
class Agent:
    ...
    def _prepare(self):
        ...
        for asin,card in self.base._dialogue_index.cards.items():
            ...
            categories=p.get('categories',[]) if p else [view[1]]
            for category in categories[-3:]:
                phrase=' '.join(words(str(category)))
                if phrase and set(phrase.split())-stop:
                    self.aliases[phrase].add(asin)
                    for token in phrase.split():
                        if token not in stop and len(token)>3:self.aliases[token].add(asin)
    ...
    def reset(self,session_id,user_profile):
        ...
        self.states[session_id]={'protocol':True,'category':'','possible':None,'constraints':[], 'messages':[],
                                 'mode':'buying','turn':0,'seen':set(),'profile':user_profile,'ask':None,'category_alias':''}
    ...
    def _state(self,state,text,turn):
        ...
        if full:
            ...
            state['seen'].clear()
        if alias:state['category_alias']=alias
    ...
    def respond(self,session_id,user_message,turn,top_k):
        if self.catalog:
            with self.catalog.lock:
                ...
                if self._version!=self.catalog.version:self._prepare()
    ...
    def _respond(self,sid,message,turn,top_k):
        ...
        if contradiction:
            state['turn']=turn;return {'message':'Those requirements conflict. Which should I keep?','ask_attribute':'other','recommendations':[]}
        ...
        candidates=self.base._fused_search(terms,100,route_limit=200,popularity_weight=.0 if state['mode']=='browsing' else 1.)
        ...
        if self.vectors is not None:
            ...
            for lane in (ranked,semantic):
                for i,a in enumerate(lane):score[a]+=1/(61+i)
        ...
        pool=ranked[:100]
        filtered=[a for a in ranked if self._allowed(a,active)]
        alias=state['category_alias']
        if alias:
            compatible=[a for a in filtered if a in self.aliases[alias]]
            if compatible:filtered=compatible
        if self.reranker is not None and filtered:filtered=self.reranker.rank(query,filtered[:20],self.base)+filtered[20:]
        ...
        self.trace[sid]={'route':route,'version':self._version,'constraints':active,'override':correction,'full_override':full,
                         'category_alias':alias,'mode':state['mode'],'confidence':confidence,'candidate_ids':pool,
