...
def train():
    rows=sorted(read_jsonl(ARTIFACTS/'datasets/train.jsonl'),key=lambda r:r['sample_id']);texts=[];labels=[]
    for row in rows:
        shopper=Shopper(row);history=''
        for turn in range(1,6):
            text=shopper.message(turn,'other');texts.append(history[-400:]+' [TURN] '+text);labels.append(label(row,turn));history+=' '+text
    ...
    classifier=CalibratedClassifierCV(LogisticRegression(max_iter=600,class_weight='balanced',random_state=20260910),cv=3)
    ...
    write_json(folder/'router-training.json',{'manifest':manifest({'labels':'buying/browsing/partial_override/category_override','calibration_cv':3},[ARTIFACTS/'datasets/train.jsonl']),
        'product_tasks':len(rows),'turns':len(texts),'training_only':True})


def score():
    from sklearn.metrics import classification_report,confusion_matrix
    rows=read_jsonl(ARTIFACTS/'datasets/dev.jsonl');router=Router();truth=[];pred=[];prob=[]
    for row in rows:
        shopper=Shopper(row);history=''
        for turn in range(1,6):
    ...
    write_json(ARTIFACTS/'learned/router-dev.json',{'manifest':manifest({},[ARTIFACTS/'datasets/dev.jsonl']),
