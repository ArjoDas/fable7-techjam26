...
import sys
import numpy as np
from extension.common import ARTIFACTS,ROOT,manifest,write_json,sha256
...
def freeze():
    ...
    candidates=[]
    for variant in ('rules','hybrid','graph','tinybert','tinybert-100','tinybert-lexical','gated-tinybert','minilm-cross','learned','residual','rag','rag-passages'):
    ...
    chosen=candidates[:2]
    write_json(path,{'manifest':manifest({'selection':'Top two full-development recovery rates, latency breaks ties; latency qualification is separate'},[canonical]),
...
def confirm():
    ...
    current=manifest({})['source_hashes']
    if current!=selection['manifest']['source_hashes']:raise RuntimeError('Serving/evaluation code changed after selection freeze')
