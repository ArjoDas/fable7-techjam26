...
def main():
    path=ROOT/'data/catalog.jsonl';store=Catalog(ARTIFACTS/'stores/official-rag',path);agent=create(store,'rag-local');ids,categories,products=catalog_index(path)
    result=evaluate(agent,read_jsonl(ROOT/'data/public_set.jsonl'),ids,categories,products)
    report={'manifest':manifest({'variant':'rag-local','catalog':'official50k'},[path,ROOT/'data/public_set.jsonl']),
    ...
    write_json(ARTIFACTS/'baselines/public-rag-local.json',report)
    ...
    print(report['aggregate'],'model calls',agent.model_calls,flush=True);agent.vectors.close();store.close()
