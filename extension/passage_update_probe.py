...
def main():
    folder=ARTIFACTS/'rag/unseen-update';store=Catalog(folder/'catalog',ROOT/'data/catalog.jsonl');vectors=PassageIndex(store);agent=RAGAgent(catalog=store,variant='weighted-rag',vectors=vectors,reranker=CrossEncoder('tinybert',limit=100))
    source=read_jsonl(ARTIFACTS/'datasets/absent-source-records.jsonl')[0];p=source['product'];a=p['parent_asin'];store.register([source])
